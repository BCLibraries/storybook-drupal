<?php

namespace Drupal\sdc_storybook\Service;

use Drupal\Component\Utility\Crypt;
use Drupal\Core\Url;
use Drupal\Core\Utility\Error;
use Drupal\sdc_storybook\Exception\StoryRenderException;
use Drupal\sdc_storybook\Exception\StorySyntaxException;
use Drupal\sdc_storybook\Story;
use Drupal\sdc_storybook\Twig\TwigExtension;
use Psr\Log\LoggerInterface;
use Twig\Environment;
use Twig\Error\LoaderError;
use Twig\Error\SyntaxError;

/**
 * Render a component with some context.
 */
final class StoryRenderer {

  /**
   * The twig environment.
   *
   * @var \Twig\Environment|null
   */
  private ?Environment $environment = NULL;

  /**
   * Creates a new ComponentRenderer.
   *
   * @param \Drupal\Core\Render\RendererInterface $drupalRenderer
   *   The drupal renderer.
   */
  public function __construct(
    private readonly LoggerInterface $logger,
  ) {}

  /**
   * Renders the Twig markup of a component.
   *
   * @param string $name
   *   The component to render.
   * @param array $context
   *   The context of the component.
   *
   * @return string
   *   The rendered markup.
   */
  public function renderStory(string $story_id, array $story_meta, string $story_template, array $context): string {
    if (!is_string($context['_story'] ?? NULL)) {
      throw new StoryRenderException('Impossible to render the story, the `_story` variable is not set in the render context.');
    }
    // Only render the story if it matches the $context['story'] requested.
    if ($story_id !== $context['_story']) {
      return '';
    }
    if (!$this->environment instanceof Environment) {
      return '';
    }
    try {
      return $this->environment->createTemplate($story_template)->render(
        array_merge($context, $story_meta)
      );
    }
    catch (LoaderError|SyntaxError $exception) {
      Error::logException($this->logger, $exception);
      return '';
    }
  }

  public function generateStoriesJsonFile(string $stories_path) {
    // Trigger the compilation of the template to collect the stories.
    $wrapper = $this->environment->load($stories_path);
    // This will execute the `compile` method without rendering anything.
    $wrapper->render(['_story' => FALSE]);
    $extension = \Drupal::service(TwigExtension::class);
    $path = ltrim(str_replace(
      \Drupal::root(),
      '',
      $wrapper->getSourceContext()->getPath(),
    ), '/');
    $wrapper_data = $extension->storyCollector->getWrapperData($path);
    $stories = array_map(
      fn(Story $story) => $this->massageStory($story, $path),
      $extension->storyCollector->getAllStories($path),
    );

    return [
      ...$wrapper_data,
      'stories' => $stories,
    ];
  }

  private function massageStory(Story $story, string $stories_path): Story {
    $meta = $story->meta;
    if ($meta['parameters']['server']['id'] ?? NULL) {
      throw new StorySyntaxException('The parameters.server.id property for a story will be generated automatically. Do not provide it.');
    }
    // This ID will be used by Storybook to call Drupal. Something like:
    // "https://example.com/$id"
    $meta['parameters']['server']['url'] = Url::fromUri('internal:/storybook/story', ['absolute' => TRUE])
      ->toString(TRUE)
      ->getGeneratedUrl();
    $meta['parameters']['server']['id'] = base64_encode(json_encode([
      'path' => $stories_path,
      'name' => $story->id,
    ], JSON_THROW_ON_ERROR));
    return new Story(
      $story->path,
      $story->id,
      $meta,
    );
  }

  private function validateStory(array $story) {}

  /**
   * Sets the Twig environment.
   *
   * @param \Twig\Environment $environment
   *   The environment.
   */
  public function setTwigEnvironment(Environment $environment): void {
    $this->environment = $environment;
  }

}
